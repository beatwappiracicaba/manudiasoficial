...
export default function MensagemArtista({ title, messages, image }) {
  return (
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          ...
          <Reveal direction="right">
            <div className="space-y-6">
              <h3 className="text-3xl font-bold text-gold">{title || 'Olá, eu sou a Manu!'}</h3>
              {messages && messages.length > 0 ? messages.map((msg, i) => (
                <p key={i} className="text-gray-300 text-lg leading-relaxed">{msg}</p>
              )) : (
                <>
