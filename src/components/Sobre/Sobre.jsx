...
export default function Sobre({ title, text, image }) {
  const displayTitle = title || 'Sobre a Manu'
  const displayText = text || 'Manu Dias vem conquistando o público com sua voz marcante e carisma, levando ao palco grandes sucessos do sertanejo e apresentações cheias de emoção.'
  const displayImage = image || SOBRE_IMG
        ...
        >
          <h2 className="text-4xl sm:text-5xl font-bold mb-4">
            {displayTitle.split(' ').map((word, i) => {
              const gold = word.toLowerCase() === 'manu'
              return (
                <span key={i}>
                  {i > 0 && ' '}
                  {gold ? (
        ...
        <div className="grid md:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          >
            <div className="relative rounded-3xl overflow-hidden shadow-2xl shadow-black/50 border border-white/10">
              <img src={displayImage} alt="Manu Dias" loading="lazy" className="w-full h-auto object-cover" />
          ...
          >
            ...
            <p className="text-gray-400 text-lg leading-relaxed">
              Conhecida como &quot;A Princesinha do Modão&quot;, ela reúne no repertório clássicos e músicas atuais, proporcionando experiências inesquecíveis ao público.
            </p>
